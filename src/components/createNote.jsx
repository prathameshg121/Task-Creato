...
function CreateArea(proc) {
  ...
  const userRoomId = localStorage.getItem("userId");
  ...
  useEffect(() => {
    setId(() => {
      console.log("SenderID changed" + senderId);
      return Math.random();
    });
  }, []);
  ...
  useEffect(() => {
    if (socket) {
      socket.on("newNote", (message) => {
        console.log("New Note for socket :" + JSON.stringify(message));
  ...
  useEffect(() => {
    if (socket) {
      socket.on("Notedata", (message) => {
        ...
        console.log(
          message.message.notedata.title +
            "===" +
            savemess.title +
            "sender" +
            senderId
        ...
        if (
          senderId !== message.message.senderId &&
          message.message.notedata.title !== savemess.title
        ) {
  ...
  function submitData(event) {
    ...
    if (isEncrypted) {
      ...
      Axios.post("/encrypt/notes/create", edata)
        .then((resdata) => {
  ...
  return (
    <div className="notesAreaDiv">
      ...
      <form className="create-note">
        ...
        <Zoom in={display}>
          <div>
            <dfn title="Add reminder">
              {" "}
              <Fab
                onClick={() => {
                  setReminder(!showReminder);
                }}
              >
                <AddAlertOutlinedIcon />{" "}
              </Fab>
            </dfn>
            <dfn title="Add Encrution">
              {" "}
              <Fab onClick={onEncryption}>
                <EnhancedEncryptionIcon />{" "}
              </Fab>{" "}
            </dfn>
            <dfn title="save">
              <Fab type="submit" onClick={submitData}>
                <PostAddIcon />{" "}
              </Fab>{" "}
            </dfn>
          </div>
